from badbigrame.badbigram import badBigramms
...
def hack(input_text):
    ...
    result += f'Размер перестановки: {currentPermutationSize}\n\n'
    ...
    while len(collumsPositions) != currentPermutationSize:  # вайл до тех пор, пока не подставим все столбцы
        for i in range(len(collums)):  # берем какой-то столбец
            if i in usedIndexes:  # Проверяем, что еще его не брали
                continue
            # подставляем справа
            bigramms = [collums[collumsPositions[len(collumsPositions) - 1]][k] + collums[i][k] for k in
                        range(len(collums[i]))]
    ...
    result += f"Перестановка: {collumsPositions}\n\n"
